...
class TestTradingBot:
    @pytest.fixture
    def bot(self):
        return TradingBot(account_balance=10000)
    
    def test_calculate_position_size(self, bot):
        size = bot.calculate_position_size(risk=2.0)
        assert size > 0
        assert size <= bot.account_balance
...
```python
def complex_trading_function(data: pd.DataFrame, signal: str) -> dict:
    """
    Execute a complex trading operation based on signals.
    
    This function analyzes market data and executes trades
    according to predefined signals and risk management rules.
    ...
    Raises:
        ValueError: Se o sinal for inválido
        ConnectionError: Se não conseguir conectar ao broker
    
    Example:
        >>> df = pd.read_csv('data.csv')
        >>> result = complex_trading_function(df, 'BUY')
